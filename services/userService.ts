...
const handleGetUserByUid = async (
	...
	try {
		...
		const user = await dbUsers.getUserByUId(
...
const handleCreateUser = async (
	...
	if (!email || !uId || !isValidUserType(user_type)) {
		return res.status(400).json({
			message:
				'Missing or invalid required fields (email, uId, user_type)',
		});
	}
...
const handleUpdateUser = async (
	...
	const { _id, email, uId, ekubo_fav_pools, user_type, remaining_requests } =
		req.body;

	if (!_id || !email || !uId || !isValidUserType(user_type)) {
		return res.status(400).json({
			message:
				'Missing or invalid required fields (_id, email, uId, user_type)',
		});
	}
...
const handleUpdateFavPools = async (
	req: NextApiRequest,
	res: NextApiResponse
) => {
...
const handleUpdateEkuboFavPools = async (
	uId,
	newEkuboFavPool,
	res: NextApiResponse
) => {
	try {
		const updatedUser = await dbUsers.updateEkuboFavPools(
			uId,
			newEkuboFavPool
		);
		if (!updatedUser) {
...
const handleDeleteFavPool = async (
	req: NextApiRequest,
	res: NextApiResponse
) => {
